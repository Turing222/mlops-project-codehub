...
check_ai_worker_imports() {
    if DEBUG=false uv run --no-dev --extra ai --extra worker python -c "import backend.worker.tasks.llm_tasks; import backend.worker.tasks.knowledge_tasks; import backend.worker.tasks.repo_analysis_tasks; import backend.worker.tasks.chat_recovery_tasks; import backend.worker.tasks.operability_tasks" >/dev/null 2>&1; then
