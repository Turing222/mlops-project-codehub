...
from app.api.dependencies import get_current_active_user, get_uow
...
# 接口：创建一个新用户
@router.patch("/users/")
async def update_user_(user_in: UserUpdate, uow: AbstractUnitOfWork = Depends(get_uow)):
    return await UserService(uow).user_update(user_in)


# 接口：通过文件上传批量插入客户
@router.("/csv_upload")
async def csv_balk_insert_users(
    uow: AbstractUnitOfWork = Depends(get_uow), file: UploadFile = File(...)
):  # noqa: B008
    # 1. 读取文件内容到内存
