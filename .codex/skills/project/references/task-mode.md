...
- `task-plan`: create or update durable work-item artifacts when the task needs persistent tracking, checkpoint updates, or dependency alignment.
...
Prefer the least invasive mode that satisfies the latest user request. If implementation intent is explicit, choose `write`, `edit`, or `add-tests`; if durable task identity is required, choose `task-plan`; if intent is unclear and risk is high, ask one concise question.
