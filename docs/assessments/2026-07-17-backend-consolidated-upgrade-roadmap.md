...
T1-4 已提供可消费的 recovery / heartbeat / outbox 事件、批次 trace 计数、持久诊断字段和 CAS replay 入口；T1-5 应直接复用其[信号交接合同](../../work-items/archive/durable-task-recovery/task-plan.md#ws7--t1-5-可观测性交接合同)，不再从自由文本日志反推任务状态。
