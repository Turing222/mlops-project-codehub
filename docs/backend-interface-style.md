...
本文档约定后端接口边界的默认写法，重点统一五件事：变量命名、错误消息回复、类型标注、注释标准、语言边界。
...
- 没有额外信息时使用 `{}`。
- 开发者日志和内部异常消息使用英文；告警、检索和聚类依赖稳定的英文 `event` / `error_code` / 结构化字段，不依赖自然语言 message。
...
- 代码侧产物使用英文：标识符、文件名、API 字段、配置 key、测试名称、开发者日志、内部异常、commit subject、`AGENTS.md`、`.codex/skills/**`、`work-items/*/manifest.yaml` 的叙述值。
- 人读项目文档使用中文正文：根 README、`docs/**`、PR 描述、review report/comment、Change Summary、`work-items/*/task-plan.md`。
- 中文正文中保留代码标识、命令、字段名、状态枚举和常用技术术语的英文原文。
- 不维护逐段双语镜像；如果代码或配置需要快速理解，只保留短模块头、中文风险提示或链接到中文文档。

`work-items/` 的边界固定为：

- `manifest.yaml` 是 agent / 机器状态源，key、枚举、状态值和正文叙述统一英文。
- `task-plan.md` 是人类叙事和交接文档，正文使用中文，技术标识保留英文。

前端目录遵循同一套语言边界；前端专属术语、文件模板和 UI 文案规则可以在 `frontend/docs/` 继续细化，但不另起一套相反规则。
